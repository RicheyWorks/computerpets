...
import { CornerDen, CornerRail } from "@/components/desk/corner-den";
import { RoomHero } from "@/components/desk/room-hero";
import { SpeciesPlaque } from "@/components/desk/species-plaque";
import { CORNER_GUIDE } from "@/lib/pets/corner-guide";
...
export const Route = createFileRoute("/corner")({
  ...
  head: () => ({
    meta: [
      { title: "The corner — ComputerPets" },
      {
        name: "description",
        content: "Ten guests of the corner on the blotter. Learn the species by watching them stay.",
      },
    ],
  }),
});
...
export function CornerPage() {
  const [selected, setSelected] = useState(CORNER_KEYS[0]!);

  return (
    <main className="bg-bg text-fg">
      <RoomHero
        room="corner"
        headline="They walk. They sit."
        line="Ten guests of the corner live on this blotter. Watch the silk. Read the plaque. Leave knowing a harvestman is not a spider, a tick is not an insect, a camel spider is not a spider, and a vinegaroon is not a scorpion."
      />

      <CornerDen selectedKey={selected} onSelect={setSelected} />

      <section className="mx-auto max-w-5xl space-y-6 px-5 py-10 sm:px-8">
        <CornerRail selectedKey={selected} onSelect={setSelected} />
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <SpeciesPlaque speciesKey={selected} />
          <aside className="rounded-[var(--radius-lg)] border border-border bg-surface p-5">
            <p className="text-[11px] uppercase tracking-[0.16em] text-subtle">How to use the corner</p>
            ...
            <p className="mt-2 text-sm text-muted">
              ...
              each guest&apos;s demo and on the desk. They walk. They sit. The plaque teaches.
            </p>
            <p className="mt-4">
