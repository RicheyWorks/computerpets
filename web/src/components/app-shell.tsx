...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={cn("bg-bg text-fg", desk || demo || live || kennelGuest || hatchery ? "h-dvh overflow-hidden" : "min-h-dvh")}>
      <header
        className={cn(
          "z-30 border-b border-border/80",
          demo || kennelGuest || hatchery
      ...
      {desk || demo || live || kennelGuest || hatchery ? (
        <div className={demo || live || kennelGuest || hatchery ? "h-dvh" : "h-dvh pt-16"}>{children}</div>
