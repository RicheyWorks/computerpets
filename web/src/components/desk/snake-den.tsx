...
function nextWaiting(onBlotter: string[], pinned: string | null) {
  return SNAKE_KEYS.find((key) => !onBlotter.includes(key) && key !== pinned) ?? null;
}
...
export function SnakeDen({
  ...
  const [onBlotter, setOnBlotter] = useState<string[]>(() => {
    const head = SNAKE_KEYS.includes(selectedKey) ? [selectedKey] : [];
    return [...head, ...SNAKE_KEYS.filter((key) => key !== selectedKey)].slice(0, ON_BLOTTER);
  });
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      setOnBlotter((current) => {
        const waiting = nextWaiting(current, selectedKey);
  ...
  return (
    <section className="relative h-[360px] overflow-hidden border-t border-border sm:h-[440px]">
      ...
      <p className="absolute bottom-4 left-5 max-w-[16rem] text-[11px] uppercase tracking-[0.18em] text-subtle">
        They crawl and stay. Tap a snake — or a name — for the plaque.
      </p>
    </section>
  );
}
