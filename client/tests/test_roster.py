...
# Same thirty wire keys as PetType / web/src/lib/pets/catalog.ts.
WEB_CATALOG = (
    "red_panda",
    "cat",
    "dog",
...
def test_roster_has_thirty_catalog_keys():
    assert len(CATALOG_KEYS) == 30
...
def test_cycle_wraps_the_full_house():
    ...
    assert next_species_key("carpet_python") == "red_panda"
    assert prev_species_key("red_panda") == "carpet_python"
