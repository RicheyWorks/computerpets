...
from computerpets_client.species import BEE_KEYS, CANOPY_KEYS, CATALOG_KEYS, CORNER_KEYS, CREEK_KEYS, FAR_KEYS, FUNGI_KEYS, GARDEN_KEYS, INSECT_KEYS, LOG_KEYS, MEADOW_KEYS, POND_KEYS, REEF_KEYS, ROOST_KEYS, SEA_KEYS, SHORE_KEYS, SNAKE_KEYS, WELL_KEYS, WOOD_KEYS, STONE_KEYS
...
def test_only_scratching_mammals_scratch():
    ...
    assert "thrash" in [a["name"] for a in acts_for("nematode")]
    assert "scud" in [a["name"] for a in acts_for("amphipod")]
    for key in SHORE_KEYS:
        names = [a["name"] for a in acts_for(key)]
        assert "scratch" not in names, key
        assert "tongue" not in names, key
    assert "wave" in [a["name"] for a in acts_for("fiddler_crab")]
    assert "run" in [a["name"] for a in acts_for("ghost_crab")]
    assert "clamp" in [a["name"] for a in acts_for("limpet")]
    assert "kick" in [a["name"] for a in acts_for("barnacle")]
    assert "graze" in [a["name"] for a in acts_for("chiton")]
    assert "rasp" in [a["name"] for a in acts_for("periwinkle")]
    assert "bury" in [a["name"] for a in acts_for("sand_dollar")]
    assert "spine" in [a["name"] for a in acts_for("sea_urchin")]
    assert "hunt" in [a["name"] for a in acts_for("knobbed_whelk")]
    ...
    assert "banner" in [a["name"] for a in acts_for("swallowtail")]
    assert "jewel" in [a["name"] for a in acts_for("jewelwing")]
    assert "lace" in [a["name"] for a in acts_for("lacewing")]
    assert "raise" in [a["name"] for a in acts_for("earwig")]
    assert "drill" in [a["name"] for a in acts_for("acorn_weevil")]
    assert "click" in [a["name"] for a in acts_for("click_beetle")]
    assert "hunt" in [a["name"] for a in acts_for("robber_fly")]
    for key in CANOPY_KEYS:
        names = [a["name"] for a in acts_for(key)]
        assert "scratch" not in names, key
        assert "tongue" not in names, key
    assert "hang" in [a["name"] for a in acts_for("sloth")]
    assert "sun" in [a["name"] for a in acts_for("lemur")]
    assert "swing" in [a["name"] for a in acts_for("gibbon")]
    assert "wrap" in [a["name"] for a in acts_for("kinkajou")]
    assert "sail" in [a["name"] for a in acts_for("colugo")]
    assert "glide" in [a["name"] for a in acts_for("flying_squirrel")]
    assert "boom" in [a["name"] for a in acts_for("howler")]
    assert "gaze" in [a["name"] for a in acts_for("tarsier")]
    assert "still" in [a["name"] for a in acts_for("potto")]
    assert "chew" in [a["name"] for a in acts_for("koala")]
    for key in REEF_KEYS:
        names = [a["name"] for a in acts_for(key)]
        assert "scratch" not in names, key
        assert "tongue" not in names, key
    assert "ridge" in [a["name"] for a in acts_for("brain_coral")]
    assert "wreath" in [a["name"] for a in acts_for("anemone")]
    assert "dart" in [a["name"] for a in acts_for("clownfish")]
    assert "scrape" in [a["name"] for a in acts_for("parrotfish")]
    assert "wave" in [a["name"] for a in acts_for("cleaner_shrimp")]
    assert "crawl" in [a["name"] for a in acts_for("sea_cucumber")]
    assert "veil" in [a["name"] for a in acts_for("lionfish")]
