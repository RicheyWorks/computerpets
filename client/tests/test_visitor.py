...
def test_visitor_identity_matches_house_formula():
    ...
    assert todays_visitor("red_panda", now).key == "silica"
    assert todays_visitor("ball_python", now).key == "nexus"
    assert todays_visitor("red_panda", datetime(2026, 1, 1)).key == "rabbit"
    assert todays_visitor("red_panda", datetime(2024, 6, 9)).key == "turkey_tail"
...
def test_visit_line_is_the_house_copy():
    ...
    assert visit_caption("red_panda", datetime(2026, 8, 17)) == "Shard may call"
