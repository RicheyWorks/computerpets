"""Study-style species rail: all two hundred ten living keys, house then snakes then tide then garden then hive then pond then roost then corner then wood then canopy then stone then creek then log then shore then reef then meadow then cellar then well then far."""
...
from .species import BEE_KEYS, CANOPY_KEYS, CATALOG_KEYS, CORNER_KEYS, CREEK_KEYS, FAR_KEYS, FUNGI_KEYS, GARDEN_KEYS, HOUSE_KEYS, INSECT_KEYS, LOG_KEYS, MEADOW_KEYS, POND_KEYS, REEF_KEYS, ROOST_KEYS, SEA_KEYS, SHORE_KEYS, SNAKE_KEYS, STONE_KEYS, WELL_KEYS, WOOD_KEYS, SPECIES, Species
...
class SpeciesRail(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        for key in WOOD_KEYS:
            row.addWidget(self._chip(SPECIES[key]))
        row.addWidget(self._label("Canopy"))
        for key in CANOPY_KEYS:
            row.addWidget(self._chip(SPECIES[key]))
        row.addWidget(self._label("Stone"))
        for key in STONE_KEYS:
            row.addWidget(self._chip(SPECIES[key]))
        row.addWidget(self._label("Creek"))
        for key in CREEK_KEYS:
            row.addWidget(self._chip(SPECIES[key]))
        row.addWidget(self._label("Log"))
        for key in LOG_KEYS:
            row.addWidget(self._chip(SPECIES[key]))
        row.addWidget(self._label("Shore"))
        for key in SHORE_KEYS:
            row.addWidget(self._chip(SPECIES[key]))
        row.addWidget(self._label("Reef"))
        for key in REEF_KEYS:
            row.addWidget(self._chip(SPECIES[key]))
        row.addWidget(self._label("Meadow"))
