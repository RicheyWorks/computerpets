...
- **Living desk** — The full house walks in `web/` and on the native overlay in `desktop/`. A PyQt6 blotter lives in `client/` (all thirty, plaques, daily weather / visitor / shed, Qt OpenGL viewport, same license contract).
...
GPU-toolkit desk — Qt OpenGL viewport, not a custom shader engine. All thirty live here with plaques, the house weather, today’s visitor, and snake sheds; unlock uses the same contract.
...
├── client/                       # PyQt6 blotter — thirty + plaques + weather/visitor/shed
